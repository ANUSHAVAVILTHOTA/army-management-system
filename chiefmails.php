...
<br><br>
 ...
 <div class="log">
<form method="POST" action="chiefcomposeaction.php" enctype="multipart/form-data" >
...
while ($row=mysqli_fetch_array($sql)) { ?>
  ...
  <option value=<?php echo $row['emailid'];?> style="color:#D2691E"><?php echo $row['fullname'] ." - ". $row['emailid'];?></option>
  <?php } ?>
  <option value=""> Please Select Agent </option> 
 <?php     
include"connection.php";
$sql=mysqli_query($db,"SELECT a.agentmailid, r.fullname FROM team t, teamassign a, register r WHERE t.id=a.teamid AND a.agentmailid=r.emailid AND t.teamlead='$email' ");
   
while ($row=mysqli_fetch_array($sql)) { ?>

  <option value=<?php echo $row['agentmailid'];?> style="color:#0000ff"><?php echo $row['fullname'] ." - ". $row['agentmailid'];?></option>
  <?php } ?> 
</select>
</div>
</div>
...
<td>
    <a href="chiefreply.php?id=<?php echo  $row['id'] ?>" class="btn btn-warning btn-sm">Reply</a>
</td>
